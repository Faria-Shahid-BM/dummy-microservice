...
def list_users() -> list[dict]:
    ...
    # Normalized so the admin table shows the scopes a login would actually
    # carry, not a stored set that implies more than it lists.
    return [
        {"username": r["username"], "scopes": _normalize_scopes(json.loads(r["scopes"]))}
        for r in rows
    ]
...
def login(body: LoginRequest):
    ...
    return {"token": token, "username": user["username"], "scopes": scopes}
